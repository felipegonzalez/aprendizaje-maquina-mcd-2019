...
```{r}
set.seed(1241)
indice_calib <- sample(1:length(prop_bosque), (length(prop_bosque)*0.5) %>% floor)
...
base_splines <- iSpline(logit(prop_bosque_calib), knots = 0, degree = 2)
matplot(logit(prop_bosque_calib), base_splines, cex = 0.2)
```
...
```{r, fig.width =4, fig.height = 3}
base_splines_eval <- iSpline(logit(prop_bosque_eval), 
                             knots = 0, degree = 2)
...
```{r}
score_brier(probs_calibradas, y_eval)
devianza <- function(p, y){
    -2*mean(y*log(p) + (1-y)*log(1 - p))
}
devianza(probs_calibradas, y_eval)
```
