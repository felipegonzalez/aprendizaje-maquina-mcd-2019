...
ggplot(calib_tbl, aes(x = prop_bosque, y = obs)) +
    geom_smooth(method = "loess", span = 0.5, se = FALSE, method.args=list(degree = 1)) + 
    geom_abline() + geom_rug()
```

y vemos que nuestro modelo no tiene una calibración muy buena: las probabilidades
...
- Podemos calibrar las probabilidades de este modelo usando regresión isotónica o regresión con splines no decrecientes. 
...
```{r}
set.seed(23441)
indice_calib <- sample(1:length(prop_bosque), length(prop_bosque)*0.8)
...
```{r, fig.width = 4, fig.height = 3}
library(splines2)
# base de splines no decrecientes:
base_splines <- iSpline(prop_bosque_calib, knots = c(1/3, 2/3), degree = 2)
...
modelo_calib <- lm(y_calib ~  -1+., data.frame(dat_calib))
...
```{r, fig.width =4, fig.height = 3}
base_splines_eval <- iSpline(prop_bosque_eval, knots = c(1/3, 2/3), degree = 2)
probs_calibradas <- predict(modelo_calib, data.frame(base_splines_eval))
calib_tbl <- tibble(prob = probs_calibradas, 
                    obs = y_eval)
ggplot(calib_tbl, aes(x = probs_calibradas, y = obs)) +
    geom_smooth(method = "loess", span = 0.5, se = FALSE, method.args=list(degree = 1)) + 
    geom_abline() + geom_rug()
```
...
2. Para el caso multiclase, se puede intentar el siguiente procedimiento: calibrar
