...
g_1 <- ggplot(calib_tbl, aes(x = prop_bosque, y = obs)) +
    ...
    geom_abline() 
g_2 <- ggplot(calib_tbl, aes(x=prop_bosque)) + geom_histogram()
...
```{r}
set.seed(1241)
indice_calib <- sample(1:length(prop_bosque), (length(prop_bosque)*0.8) %>% floor)
...
modelo_calib <- lm(y_calib ~ ., data.frame(dat_calib))
...
g_2 <- ggplot(calib_tbl, aes(x = probs_calibradas)) + geom_histogram()
grid.arrange(g_1, g_2, heights = c(4, 2))
```
...
y evaluación.
3. Para el caso multiclase, se puede intentar el siguiente procedimiento: calibrar
