...
- *Si quitamos una variable, y el error de predicción no se degrada, la variable no es importante*.
...
variables pueden hacer su trabajo, y el modelo no se degrada mucho. Sin embargo,
si elimináramos las otras variables, esta variable influiría mucho en la predicción.

- *Si las predicciones cambian mucho cuando una variable cambia, entonces la variable es importante*. 
Este concepto funciona mejor, al menos desde el punto de vista predictivo, y es
posible refinarlo. Su defecto es que debemos decidir qué cambios queremos medir. Si el modelo es simple (por ejemplo, lineal), entonces es relativamente fácil usar cambios marginales. Pero en modelos no lineales cambios marginales no necesariamente evalúan 
...
ggplot(importancias_casas_tbl, aes(x = name, y = valor_porc)) + 
    geom_point() + coord_flip() + ylab("Decremento porcentual en Error")
```
...
- El parámetro más importante de afinar es usualmente $mtry$, el número de variables que se escogen
