...
mod_casas <- rfinterval(SalePrice ~ Neighborhood + MSSubClass +
            OverallQual + OverallCond + YearBuilt + GrLivArea +
            BsmtQual + TotalBsmtSF + GarageQual, 
       method = "quantreg", 
       params_ranger = list(mtry = 4, num.trees = 1500), 
       alpha = 0.05, 
       train_data = casas_entrena, test_data = casas_prueba)
```

```{r}
intervalos <- mod_casas$quantreg_interval
preds <- mod_casas$testPred
