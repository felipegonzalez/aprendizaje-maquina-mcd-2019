...
ggplot(tibble(x = seq(0, 1, 0.01)), aes(x = x)) + stat_function(fun = p_1)  +
    ylab("p_1")
```
...
graf_1 <- ggplot(dat_ent, aes(x = x)) +
  geom_jitter(aes(colour = factor(g), y = as.numeric(g=='1')), 
    width=0.02, height=0.05) + ylab("") + labs(colour = "Clase")
graf_1 
```
...
```{r,fig.width = 5, fig.asp = 0.7}
graf_data <- data_frame(x = seq(0, 1, 0.01))
...
dat_dev$hat_p_1 <- predict(vmc_entrena, type ='prob')[ ,2]
dat_dev$hat_p_2 <- predict(vmc_entrena, type ='prob')[ ,1]
...
dat_dev_prueba$hat_p_1 <- predict(vmc_prueba, type ='prob')[,2]
dat_dev_prueba$hat_p_2 <- predict(vmc_prueba, type ='prob')[,1]
