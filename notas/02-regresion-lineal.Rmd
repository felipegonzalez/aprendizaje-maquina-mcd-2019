...
beta <- c(100, 25, 20) # interpreta estos coeficientes
predecir <- preds_fun(beta)
...
- El coeficiente de sótano se interpreta así: si en una casa dada, todo los demás constante, se aumenta en 10 m2 el sótano, la predicción de precio aumenta en 2.3 mil dólares.

- Todo lo demás constante, el efecto de área habitable depende de la calidad:
    - Para calidad = 0, un aumento de 100 m2 equivale a un aumento de unos 28 mil dólares en la predicción precio de venta
    - Si la calidad sube un punto, entonces el incremento por cada 100m2 sube 19 mil dòlares adicionales la predicción
    - Si la calidad decrece en un punto, entonces el incremento por cada 100m2 decrece 19 mil dólares adicionales en la predicción
...
Otros cuidados adicionales, aún más sutiles e importantes, se requieren si queremos hacer afirmaciones causales:
...
```{r, message = FALSE}
modelo_casas <- lm(precio_miles ~ habitable_100_m2 + calidad_m2 + GarageArea +
                       BsmtFinSF1,
                   data = casas_ent, weights = 1 / habitable_100_m2^2)
preds_val <- predict(modelo_casas, newdata = casas_val, interval = "prediction",
                     level = 0.95,
                     weights = 1 / casas_val$habitable_100_m2^2)
valida_g <- bind_cols(preds_val %>% as_tibble, casas_val %>% select(precio_miles))
head(valida_g)
