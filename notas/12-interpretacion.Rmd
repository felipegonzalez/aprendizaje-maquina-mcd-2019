...
Usaremos como referencias [este libro](https://christophm.github.io/interpretable-ml-book/index.html) y [@ESL].
...
que es una función de $m$ variables, donde fijamos los valores de las variables en $x_s$ y promediamos sobre los datos de entrenamiento para las entradas que están en $x_c$.
...
**Ojo**: Para evitar sobreinterpretar extrapolaciones en estas gráficas, podemos restringir agregar
...
Mostramos cómo las gráficas de dependencia parcial pueden ser poco interpretables,
primero en el caso en que hay una interacción fuerte entre variables fijas y promediadas. En rojo
...
```{r, fig.width =6, fig.height= 9, cache = TRUE}
set.seed(12)
...
- La importancia por permutaciones muestra en qué variables se está apoyando el modelo para
...
definir la contribución $\phi_j (x)$ de la variable $j$ en la predicción como

$$\phi_j (x) = \beta_j(x_j - \mu_j),$$
...
- Si una variable satisface $\phi_W (x) = \phi_{W,j} (x)$ para cualquier subconjunto de variables $W$,
...
y entonces la contribución en este caso es:
$$v(1,2) - v(2) = \beta_1 (x_1^* - \bar{x_1})$$
que es la misma cantidad. Así que no importa como "incluyamos" la variable 1 bajo este criterio,
...
shap_nse <- xgb.plot.shap(mat_x, model = boost_nse, top_n = 9, n_col = 2)
