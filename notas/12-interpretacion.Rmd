...
1- **Entendimiento del problema de interés**: queremos aprender o encontrar hallazgos que nos ayuden a entender cómo funciona el fenómeno que nos interesa (por ejemplo, por qué la gente cancela su cuenta, por qué alguien cae en impago, por qué una casa cuesta lo que cuesta, etc.) Esta es una interpretación al nivel del proceso que genera los datos.

En este punto, incluso los modelos más simples frecuentemente no son interpretables.


2: **Entendimiento del funcionamiento de una familia de modelos**: en este caso, quisiéramos entender cómo se comportan distintas estructuras predictivas para una familia de modelos dada y un problema dado. Esto incluye inclusión o exclusión de variables, ingeniería de entradas, búsquedas de estructuras parsimoniosas.

2. **Entendimiento del funcionamiento de un modelo dado**: queremos entender cómo funciona un modelo ajustado:  ¿cuándo es probable que alguien caiga en 
...
En el segundo tipo de interpretación buscamos entender la *estructura predictiva*
de un problema dada una familia posible de modelos. Este problema se puede aproximar
mediante ideas estadísticas, y buscamos contestar, por ejemplo: ¿cómo podemos describir la variación de los modelos respecto a la muestra de entrenamiento? Este enfoque es especialmente útil cuando utilizamos modelos que pretenden reflejar el proceso generador de los datos, de modo que usualmente pensamos que tenemos la "familia de modelos correcta", o algo muy cercano.


## Interpretación de predicciones

El tercer tipo de interpretación, que tiene qué ver cómo construye predicciones
particulares un modelo es un problema en general considerablemente más simple. Este problema tiene relevancia cuando las predicciones son insumo de una de una decisión, y quisiéramos saber cómo influyen las variables individuales en la predicción para dar
transparencia a la decisión (por ejemplo, ¿por qué rechazaron mi crédito? ¿Por qué un 
transacción es marcada como posible fraude?).

Existen varias herramientas útiles para atacar este problema. Veremos:
...
reps <- map(1:5, ~ replica_boot(casas_prueba, bosque_casas, SalePrice)) %>% bind_rows()
...
Una tarea común que se puede requerir para transparentar las predicciones
...
Definimos en primer lugar la predicción que haríamos si solamente conocemos
un subconjunto $W$ de las variables $X_1,\ldots, X_p$. Este valor es

$$f_W(x) = E[f | X_i = x_i, i\in W]$$

y definimos $f_\emptyset (x)$ como la constante igual a $E(f)$ (la predicción promedio).
Podemos definir entonces 

$$\Delta_W (x) = f_W(x) - f_\emptyset (x)$$
como la *contribución del subconjunto de variables en $W$ a la predicción $f(x)$.

### Ejemplo {-}
Para un modelo lineal sin interacciones, se puede confirmar sin dificultad que
$$\Delta_W (x) = \sum_{i\in W} \beta_i(x_i - \mu_i)$$ 
para cualquier $x$. Nótese que si definimos $\phi_i(x) = \beta_i (x_i - \mu_i),$
entonces podemos calcular fácilmente 

### Ejemplo {-}
Ahora consideramos un modelo lineal con interacciones, por ejemplo
$$f(x_1, x_2) = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \beta_3 x_1 x_2,$$
¿Cómo se ven las contribuciones $\Delta_W$? Tenemos que
$$\Delta_{1}(x) = \beta_1x_1   - \beta_3x_1E(X_1X_2) $$
y vemos que la contribución de $x_1$ depende


Una manera de abordar este problema consiste en pensar qué propiedades
deseables deberían
cumplir las contribuciones $\phi_i(x)$ de cada variable $x_i$, desde el punto
de vista de su interpretación. 

1. La suma de las contribuciones individuales $\phi_i(x)$ satisface
$$\sum_i \phi_i(x) = f(x) - E(f(X)),$$
es decir, la suma de las contribuciones es la diferencia entre la predicción $f(x)$ y la predicción promedio $E(f(X))$.

2. 


Una primera manera de abordar esto es considerar las contribuciones
$$\phi_i(x) = f(x_1,\ldots, x_p)  - E(f(x_1, \ldots, X_i, \ldots, x_p)).$$
Esta última cantidad es la diferencia entre la predicción menos el valor esperado
de la predicción promediando sobre la variable $X_i$. Esta cantidad, sin
...
Supongamos que tenemos un modelo aditivo sin interacciones 
$$f(X_1,X_2,\ldots, X_p) = \beta_0 +h_1(X_1) +\cdots h(X_p)$$ 

Si hacemos una predicción particular para $x = (x_1,x_2,\ldots, x_p)$, podemos
considerar el valor 

$$\phi_i (x) = \beta_ix_i - \beta_i \mu_i,$$
donde $\mu_i$ es la media de la variable $X_i$. Esta puede pensarse
como una medida de la contribución de $x_i$ a la predicción respecto a su valor esperado,
y estos valores cumplen
$$\sum_i \phi_i (x) = f(x) - f(\mu),$$
donde $\mu = (\mu_i, \ldots, \mu_p)$. Es decir las *contribuciones* $\phi_i$ explican
la diferencia de la predicción $f(x_i)$ en relación a la predicción en los valores
esperados.
...
```{r}
shapley = Shapley$new(predictor, x.interest = x[21,])
shapley$plot()
```