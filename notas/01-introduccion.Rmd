...
soluciones forman parte de un ciclo de **análisis de datos** donde podemos aprender de una forma más concentrada cuáles son
...
fotos tomadas por los usuarios para medir el consumo?

Una solución a este problema podría resolverse si personas etiquetaran las fotografías, pero eso puede ser costoso. Quisiéramos procesar de manera automática las fotografías y
producir una buena estimación del consumo. Por ejemplo, consideremos:

```{r, message = FALSE, out.width = '500px'}
library(imager)
library(tidyverse)
medidor_vid <- load.video("figuras/gauge_raw.mp4", fps = 5)
plot(medidor_vid, frame = 20, axes = FALSE)
```

Nótese que las imágenes y videos son matrices o arreglos de valores de pixeles:
```{r}
dim(medidor_vid)
...
ggplot(angulos_tbl, aes(x = t, y = angulo)) + geom_line() + geom_point()
```

En el **enfoque de aprendizaje**, comenzamos con un conjunto de datos etiquetado
...
```{r}
# los datos auto son del paquete ISLR
