...
```{r, eval = FALSE}
plots <- map(1:71, 
  ~ ggplot(angulos_tbl %>% filter( t <= .x),  aes(x = t, y = angulo)) + 
  geom_line() + geom_point() + xlim(c(1,71)) + ylim(c(70, 360)))

```

```{r chunk-label, eval = FALSE, fig.show='animate', ffmpeg.format='gif', dev='jpeg', interval=0.2}
walk(plots, print)
```
...
dat_ingreso <- read_csv(file = '../datos/enigh-ejemplo.csv') %>% 
  ...
  rename(personas_ocupadas = PEROCU)

sample_n(dat_ingreso, 10) %>% 
    select(NOM_ENT..307, num_focos, marginación, 
           tel_celular, personas_ocupadas, ingreso_miles)  %>%
...
```{r, message = FALSE, warning = FALSE, fig.width = 8, fig.asp = 0.3}
ggplot(dat_ingreso, 
       aes(x = personas_ocupadas, y = ingreso_miles, colour= tel_celular)) + 
...
```{r, fig.width = 8, fig.asp = 0.3, message = FALSE, warning = FALSE}
ggplot(dat_ingreso, 
...
con alto número de personas ocupadas? Claramente, es necesario incluír
