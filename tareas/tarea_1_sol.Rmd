...
```{r, warning = FALSE}
## Una solución
replicaciones <- map_dfr(1:100, function(i){
  datos_entrena <- simula_muestra(f, x)
  modelos <- ajusta_modelos(datos_entrena)
  error_entrena <- error_f(datos_entrena)
  error_prueba <- error_f(datos_prueba)
  ...
  errores <- modelos %>% 
    mutate(error_ent = map_dbl(modelo, error_entrena),
           error_pr = map_dbl(modelo, error_prueba))
  errores <- errores %>% mutate(replicacion = i)
  errores
}) %>% gather(tipo, valor, error_ent:error_pr)

ggplot(replicaciones, aes(x = nombre, y = valor, colour= tipo)) +
  geom_boxplot()
```